package com.briup.crm.gateway.filter;
...
//            }
            ...
            if (StringUtils.isEmpty(token)) {
                return unauthorizedResponse(exchange, serverHttpResponse, ResultCode.TOKEN_MISSION);
            }
...
//        }
     ...
     * 将 JWT 鉴权失败的消息响应给客户端
     */
    private Mono<Void> unauthorizedResponse(ServerWebExchange exchange, ServerHttpResponse serverHttpResponse, ResultCode codeStatus) {
        log.error("[鉴权异常处理]请求路径:{}", exchange.getRequest().getPath());
        serverHttpResponse.setStatusCode(HttpStatus.UNAUTHORIZED);
        serverHttpResponse.getHeaders().add("Content-Type", "application/json;charset=UTF-8");

        Result responseResult = Result.failure(codeStatus.code(), codeStatus.message());

        DataBuffer dataBuffer = serverHttpResponse.bufferFactory()
                .wrap(new Gson().toJson(responseResult)
